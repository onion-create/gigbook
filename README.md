...
China has over **84 million gig workers** — delivery riders, couriers, ride-hail drivers navigating the **零工经济**. They log every ride, every delivery, every expense in their head or on scraps of paper. The accounting apps that exist — 随手记, 鲨鱼记账, 挖财 — were all built for *consumer spending*. "Where did my money go?" is not a gig worker's question.
...
| 💰 **多平台收入追踪** | 美团、饿了么、闪送、顺丰同城、货拉拉、滴滴、跑腿 — 7 大主流平台 + 自定义分类。每个平台独立图标和颜色标识，自动生成平台分布图。 |
...
| 🌓 **深色模式** | 完整深色模式覆盖 54 个 CSS 变量。卡片、标题、按钮、输入框、图表——全元素适配。JavaScript 强制 header 渐变，彻底解决缓存导致的渲染异常。 |
...
| 🎨 **设计系统** | 暖白色调（Linear/Arc 风格），5 层阴影深度系统，毛玻璃 header，渐变色进度条。所有交互元素使用 cubic-bezier 过渡动画。配色由 CSS 变量驱动（54 个变量），深色模式一键切换。 |
...
| Design System | CSS custom properties (54 variables) |
